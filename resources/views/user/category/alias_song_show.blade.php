    ...
    <div class="container-fluid">
        ...
        @if (isset($songsInPlaylists) && $songsInPlaylists->count() > 1)
            @php
                ...
                $nextSong =
                    $currentIndex < $songsInPlaylists->count() - 1 ? $songsInPlaylists[$currentIndex + 1] : null;
            @endphp

            <div class="vachanamrut-navigation">
                <!-- Previous Button -->
                @if ($previousSong)
                    <a href="{{ route('user.categories.aliasSongShow', ['alias' => $category->alias, 'id' => $previousSong->song_code]) }}"
                ...
                <div class="nav-center">
                    {{ $currentIndex + 1 }} {{ __('of') }} {{ $songsInPlaylists->count() }}
                </div>

                <!-- Next Button -->
                @if ($nextSong)
                    <a href="{{ route('user.categories.aliasSongShow', ['alias' => $category->alias, 'id' => $nextSong->song_code]) }}"
            ...
            <!-- Optional: Keep the numbered navigation as well (smaller version) -->
            <div class="vachanamrut-nav" style="margin-top: 0.5rem;">
                @foreach ($songsInPlaylists as $index => $playlistSong)
                    <a href="{{ route('user.categories.aliasSongShow', ['alias' => $category->alias, 'id' => $playlistSong->song_code]) }}"
...
@section('script')
    <script>
        ...
        // on click backspace go to specific route
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Backspace') {
                window.location.href = '{{ route('user.categories.aliasShow', ['alias' => $category->alias]) }}';
            }
        });
    </script>
