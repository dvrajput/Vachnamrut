...
$(document).ready(function() {
    // Initialize DataTable with FIXED sorting arrows
    const table = $('#songsTable').DataTable({
        ...
        columns: [
            {
                data: 'song_code',
                name: 'song_code',
                orderable: false, // FIXED: Disable sorting to remove arrows
                width: '10%'
            },
            {
                data: 'title_en',
                ...
                render: function(data, type, row) {
                    return data ? `<span class="text-truncate" title="${data}">${data.length > 30 ? data.substr(0, 30) + '...' : data}</span>` : '<span class="text-muted">—</span>';
                }
            },
            {
                data: 'lyrics_en',
            ...
            },
            {
                ...
                width: '25%',
                render: function(data, type, row) {
                    return data ? `<span class="text-truncate gujarati-text" title="${data}">${data.length > 30 ? data.substr(0, 30) + '...' : data}</span>` : '<span class="text-muted">—</span>';
                }
            },
            {
                data: 'lyrics_gu',
                ...
                render: function(data, type, row) {
                    let actionHtml = `
                        <div class="btn-group" role="group" aria-label="Actions">
                            <a href="{{ url('admin/songs') }}/${row.song_code}" 
                               class="btn btn-sm btn-info" 
                               data-bs-toggle="tooltip" 
                               title="{{ __('View Vachanamrut') }}">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="/admin/songs/${row.song_code}/edit" 
                    ...
                    if (deleteBtn === '1') {
                        actionHtml += `
                            <form action="{{ url('admin/songs') }}/${row.song_code}" 
                                  method="POST" 
                                  style="display:inline;"
                                  onsubmit="return confirmDelete('${row.song_code}')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
        ...
        pageLength: 25,
        responsive: true,
        // REMOVED: Default sorting to prevent arrows
    ...
    window.confirmDelete = function(code) {
        return confirm(`{{ __('Are you sure you want to delete Vachanamrut') }} "${code}"? {{ __('This action cannot be undone.') }}`);
    };
